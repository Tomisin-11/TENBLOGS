...
/* Tug-of-war bar: home fills from left, away from right, sum = 100% */
const DualBar = ({ hv, av, homeColor, awayColor }) => {
  ...
  return (
    <div style={{ height:6, display:'flex', overflow:'hidden', borderRadius:1,
      ...
      <div style={{ width:`${hPct}%`, background:homeColor, transition:'width 0.7s', height:'100%' }} />
      <div style={{ width:`${aPct}%`, background:awayColor, transition:'width 0.7s', height:'100%', marginLeft:'auto' }} />
    </div>
  )
}
...
export default function H2HCard({ d }) {
  ...
  // Attacking threat: always sums to 100%
  const rawHome = parseFloat(d.homeAttackingThreat) || 0
  const rawAway = parseFloat(d.awayAttackingThreat) || 0
  const total = rawHome + rawAway || 100
  const homeThreat = Math.round((rawHome / total) * 100)
  const awayThreat = 100 - homeThreat
    ...
    <div id="preview-h2h"
      ...
      <div className="px-4 py-4">
        {[
          ...
          { lbl:'Avg Possession',   hv:d.homePoss,  av:100-parseFloat(d.homePoss||50), dispHv:(d.homePoss||50)+'%', dispAv:(100-parseFloat(d.homePoss||50))+'%' },
          { lbl:'Attacking Threat', hv:homeThreat, av:awayThreat, dispHv:homeThreat+'%', dispAv:awayThreat+'%' },
